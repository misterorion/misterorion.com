...
externalTelemetryAttributes) {
  ...
  queryIds.pageQueryIds = queryIds.pageQueryIds.filter(query => (0, _pageMode.getPageMode)(query) === `SSG`); // Start saving page.mode in the main process (while queries run in workers in parallel)

  const waitMaterializePageMode = (0, _pageMode.materializePageMode)();
  let waitForWorkerPoolRestart = Promise.resolve(); // If one wants to debug query running you can set the CPU count to 1
  ...
  await waitMaterializePageMode;
  const waitWorkerPoolEnd = Promise.all(workerPool.end()); // create scope so we don't leak state object

  {
    ...
    const graphqlTypegenOptions = config.graphqlTypegen; // Only generate types when the option is enabled
