...
async function parseToAst(filePath, fileStr, {
  ...
  let ast; // Since gatsby-plugin-mdx v4, we are using the resourceQuery feature of webpack's loaders to inject a content file into a page component.

  const cleanFilePath = (0, _gatsbyCoreUtils.getPathToLayoutComponent)(filePath); // Preprocess and attempt to parse source; return an AST if we can, log an
  // error if we can't.
