...
${reservedFields.map(f => `  * "${f}"`).join(`\n`)}
  ...
  if (isNotTestEnv) {
    if (pageComponentCache.has(page.component)) {
      ...
      const splitPath = splitComponentPath(page.component); // normalize component path

      page.component = slash(splitPath[0]); // check if path uses correct casing - incorrect casing will
      ...
      try {
        ...
        const commonDir = getCommonDir(directory, page.component); // using `path.win32` to force case insensitive relative path
