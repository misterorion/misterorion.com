...
} = require(`gatsby-core-utils`); // A file can be shadowed by a file of the same extension, or a file of a
...
module.exports = class GatsbyThemeComponentShadowingResolverPlugin {
  constructor({
    ...
    this.extensionsCategory = { ...DEFAULT_FILE_EXTENSIONS_CATEGORIES,
      ...extensionsCategory
    };
  ...
  apply(resolver) {
    ...
    resolver.getHook(`before-resolved`).tapAsync(`GatsbyThemeComponentShadowingResolverPlugin`, (request, stack, callback) => {
      ...
      const originalRequestComponent = path.basename(originalRequestPath); // This is the shadowing algorithm.
  ...
  resolveComponentPath({
    ...
    for (const theme of themesArray) {
      const possibleComponentPath = path.join(theme, getPathToLayoutComponent(component));
      ...
      const existsDir = dir.map(filepath => path.basename(filepath)); // if no exact path, search for extension

      const matchingShadowFile = acceptableShadowFileNames.find(shadowFile => existsDir.includes(shadowFile));

      if (matchingShadowFile) {
        return path.join(path.dirname(possibleComponentPath), matchingShadowFile);
      }
    }
  ...
  getThemeAndComponent(filepath) {
    ...
    const allMatchingThemes = this.themes.filter(({
      themeDir
    }) => filepath.startsWith(path.join(themeDir, `src`))); // The same theme can be included twice in the themes list causing multiple
    // matches. This case should only be counted as a single match for that theme.

    const matchingThemes = _.uniqBy(allMatchingThemes, `themeName`); // 0 matching themes happens a lot for paths we don't want to handle
    ...
    const theme = matchingThemes[0]; // get the location of the component relative to its theme's src/
  ...
  requestPathIsIssuerShadowPath({
    ...
    const shadowFiles = this.getBaseShadowDirsForThemes(theme.themeName).concat(path.join(userSiteDir, `src`, theme.themeName)).map(dir => path.join(dir, component)).flatMap(comp => this.getAcceptableShadowFileNames(comp)); // if the issuer is requesting a path that is a potential shadow path of itself
