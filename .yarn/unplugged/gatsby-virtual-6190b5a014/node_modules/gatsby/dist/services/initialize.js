...
async function initialize({
  ...
  activity.end(); // GATSBY_QUERY_ON_DEMAND_LOADING_INDICATOR=false gatsby develop
  ...
  const cacheDirExists = lmdbCacheDirExists; // For builds in case public dir exists, but cache doesn't, we need to clean up potentially stale
  ...
  const pluginVersions = flattenedPlugins.map(p => p.version); // we should include gatsby version as well
  ...
  if (!oldPluginsHash || pluginsHash !== oldPluginsHash || cacheIsCorrupt) {
    try {
      ...
      `.cache/**`, `.cache/data/**`, `!.cache/data/gatsby-core-utils/**`, `!.cache/compiled`, // Add webpack
      `!.cache/webpack`];
  ...
  if (state.config.graphqlTypegen) {
    _gatsbyTelemetry.default.trackFeatureIsUsed(`GraphQLTypegen`); // This is only run during `gatsby develop`
