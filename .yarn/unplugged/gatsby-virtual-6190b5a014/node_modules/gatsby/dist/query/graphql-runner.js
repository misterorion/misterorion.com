...
class GraphQLRunner {
  ...
  async query(query, context, {
    ...
    if (this.stats) {
      ...
      this.stats.uniqueQueries.add(_crypto.default.createHash(`sha1`).update(queryText).digest(`hex`));
    }
    ...
    const {
      ...
      warnings,
      document
    } = this.validate(schema, queryText, this.parse(query)); // Queries are usually executed in batch. But after the batch is finished
