...
module.exports = async (program, directory, suppliedStage, port, {
      ...
      case `develop`:
        {
          ...
          const isCustomEslint = (0, _localEslintConfigFinder.hasLocalEslint)(program.directory); // if no local eslint config, then add gatsby config
  ...
  function getModule() {
    ...
    // @see https://github.com/webpack/webpack/issues/11467
    {
      ...
      modulesThatUseGatsby
    }), rules.yaml(), rules.fonts(), rules.images(), rules.media(), rules.miscAssets()]; // TODO(v5): Remove since this is only useful during Gatsby 4 publishes
  ...
  if (stage === `build-html` || stage === `develop-html`) {
    ...
    const shouldMarkPackagesAsExternal = stage === `develop-html` || !(0, _enginesHelpers.shouldGenerateEngines)(); // tracking = build-html (when not generating engines)

    const shouldTrackBuiltins = stage === `build-html` && !(0, _enginesHelpers.shouldGenerateEngines)(); // removes node internals from bundle
