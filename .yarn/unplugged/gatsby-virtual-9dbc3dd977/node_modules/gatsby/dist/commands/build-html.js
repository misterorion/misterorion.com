...
exports.deleteRenderer = exports.buildRenderer = exports.buildPartialHydrationRenderer = void 0;
...
const buildPartialHydrationRenderer = async (program, stage, parentSpan) => {
  ...
  config.cache = false;
  config.output.path = path.join(program.directory, `.cache`, `partial-hydration`); // TODO collect javascript aliases to match the partial hydration bundle
...
const renderPartialHydrationQueue = async (workerPool, activity, pages) => {
  // We need to only pass env vars that are set programmatically in gatsby-cli
  // to child process. Other vars will be picked up from environment.
  const envVars = [[`NODE_ENV`, process.env.NODE_ENV], [`gatsby_executing_command`, process.env.gatsby_executing_command], [`gatsby_log_level`, process.env.gatsby_log_level]];
  const segments = (0, _chunk2.default)(pages, 50);
  const sessionId = Date.now(); // const { webpackCompilationHash } = store.getState()
...
const buildHTML = async ({
  ...
  if ((process.env.GATSBY_PARTIAL_HYDRATION === `true` || process.env.GATSBY_PARTIAL_HYDRATION === `1`) && "4" === `5`) {
...
async function buildHTMLPagesAndDeleteStaleArtifacts({
  ...
  if ((process.env.GATSBY_PARTIAL_HYDRATION === `true` || process.env.GATSBY_PARTIAL_HYDRATION === `1`) && "4" === `5`) {
    if (toRegenerate.length > 0) {
      ...
      try {
        buildHTMLActivityProgress.start();
        await renderPartialHydrationQueue(workerPool, buildHTMLActivityProgress, toRegenerate);
      } catch (error) {
  ...
  if ("4" !== `5` && !program.keepPageRenderer) {
    try {
